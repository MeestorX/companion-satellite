...
export function handleWiFi(router: Router) {
	...
	function netmaskToCIDR(netmask: string): number {
		...
		const binaryNetmask = netmaskParts
	...
	router.get('/api/wifi/status', async (ctx) => {
		try {
			...
			const activeNetwork = lines
				.map((line) => line.split(':'))
				.find(([name,type,id,dev]) => (dev === 'wlan0' || id.includes('wireless')))
	...
	router.post('/api/wifi/connect', koaBody(), async (ctx) => {
		...
		try {
			...
			if (details.includes('wireless-security.psk') || details.includes('could not be found')) details = 'Unknown SSID or Incorrect Password'
	...
	router.post('/api/wired/configure', koaBody(), async (ctx) => {
		...
		try {
			let cidr = netmaskToCIDR(subnetMask)
